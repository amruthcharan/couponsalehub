...
class CouponController extends VoyagerBaseController
{
    public function index(Request $request)
    {
        ...
        if (strlen($dataType->model_name) != 0) {
            ...
            $query = $model->whereStoreId($store_id);
...
//        $dataTypeContent= $dataTypeContent->where('store_id', '==', $store_id);
        // Actions
        $actions = [];
    ...
    public function store(Request $request)
    {
        ...
        if (!$request->has('_tagging')) {
            if (auth()->user()->can('browse', $data)) {
                $redirect = redirect()->to("/admin/coupons?store_id=".$request->get('store_id'));
            } else {
...
//            $model = $model->{$dataType->scope}($store->id);
//        }
        if ($model && in_array(SoftDeletes::class, class_uses_recursive($model))) {
